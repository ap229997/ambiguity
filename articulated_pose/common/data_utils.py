...
def crop_and_pad(img, bbox, args, scale=1):
    if bbox is None: # resize the image and return
        img_crop = generate_patch_image_clean(img.transpose(1,2,0), [args.img_res/2, args.img_res/2, args.img_res, args.img_res], 
    ...
    img_crop = generate_patch_image_clean(img.transpose(1,2,0), [x_mid, y_mid, size*scale, size*scale], 1.0, 0.0, [args.img_res_ds, args.img_res_ds], cv2.INTER_CUBIC)[0]
    img_crop = np.clip(img_crop, 0, 1)
    new_bbox = np.array([x_mid-(size*scale)//2, y_mid-(size*scale)//2, x_mid+(size*scale)//2, y_mid+(size*scale)//2]).clip(0, args.img_res-1).astype(np.int16) # np.array([x0, y0, x1, y1])
