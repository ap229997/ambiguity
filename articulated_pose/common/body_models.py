...
class MANODecimator:
    def __init__(self):
        data = np.load(
            f"{os.environ['DATA_DIR']}/data/arctic_data/data/meta/mano_decimator_195.npy", allow_pickle=True
        ).item()
        mydata = {}
...
MODEL_DIR = f"{os.environ['DATA_DIR']}/data/body_models/mano"
...
MANO_MODEL_DIR = f"{os.environ['DATA_DIR']}/data/body_models/mano"
SMPLX_MODEL_P = {
    "male": f"{os.environ['DATA_DIR']}/data/body_models/smplx/SMPLX_MALE.npz",
    "female": f"{os.environ['DATA_DIR']}/data/body_models/smplx/SMPLX_FEMALE.npz",
    "neutral": f"{os.environ['DATA_DIR']}/data/body_models/smplx/SMPLX_NEUTRAL.npz",
}
...
def build_subject_smplx(batch_size, subject_id):
    with open(f"{os.environ['DATA_DIR']}/data/arctic_data/data/meta/misc.json", "r") as f:
        misc = json.load(f)
    vtemplate_p = f"{os.environ['DATA_DIR']}/data/arctic_data/data/meta/subject_vtemplates/{subject_id}.obj"
