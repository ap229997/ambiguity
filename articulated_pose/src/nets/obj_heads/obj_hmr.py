...
class ObjectHMR(nn.Module):
    def __init__(self, feat_dim, n_iter, **kwargs):
        super().__init__()

        obj_specs = {"rot": 3, "cam_t/wp": 3, "radian": 1}
        self.hmr_layer = HMRLayer(feat_dim, 1024, obj_specs, **kwargs)
    ...
    def init_vector_dict(self, features):
        ...
        if features.ndim == 4:
            init_transl = self.cam_init(self.avgpool(features).view(batch_size, -1))
        else:
            init_transl = self.cam_init(features)
