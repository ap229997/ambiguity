...
def do_test(cfg, model, iteration='final', storage=None):
    ...
    for dataset_name in dataset_names_test:
        ...
        if comm.is_main_process():
            ...
            # store eval results in storage for tensorboard logging
            category_names = ['car', 'pedestrian', 'truck', 'van', 'cyclist']
            for cat in category_names:
                cat_2D, cat_3D = (np.nan,) * 2
                if 'AP-{}'.format(cat) in eval_helper.results[dataset_name]['bbox_2D']:
                    cat_2D = eval_helper.results[dataset_name]['bbox_2D']['AP-{}'.format(cat)]
                    cat_3D = eval_helper.results[dataset_name]['bbox_3D']['AP-{}'.format(cat)]
                if not np.isnan(cat_2D) or not np.isnan(cat_3D):
                    # results_cat[cat] = {"AP2D": cat_2D, "AP3D": cat_3D}
                    storage.put_scalar(f"AP2D/{cat}", cat_2D)
                    storage.put_scalar(f"AP3D/{cat}", cat_3D)

            storage.put_scalar(f"AP2D/AP", eval_helper.results[dataset_name]['bbox_2D']['AP'])
            storage.put_scalar(f"AP2D/AP50", eval_helper.results[dataset_name]['bbox_2D']['AP50'])
            storage.put_scalar(f"AP2D/AP75", eval_helper.results[dataset_name]['bbox_2D']['AP75'])
            storage.put_scalar(f"AP2D/AP95", eval_helper.results[dataset_name]['bbox_2D']['AP95'])

            storage.put_scalar(f"AP3D/AP", eval_helper.results[dataset_name]['bbox_3D']['AP'])
            # storage.put_scalar(f"AP3D/AP15", eval_helper.results[dataset_name]['bbox_3D']['AP15'])
            storage.put_scalar(f"AP3D/AP25", eval_helper.results[dataset_name]['bbox_3D']['AP25'])
            storage.put_scalar(f"AP3D/AP50", eval_helper.results[dataset_name]['bbox_3D']['AP50'])
            storage.put_scalar(f"AP3D/AP70", eval_helper.results[dataset_name]['bbox_3D']['AP70'])
...
def do_train(cfg, model, dataset_id_to_unknown_cats, dataset_id_to_src, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        ...
        for _ in tqdm(range(max_iter - start_iter)):
            data = next(data_iter)
            ...
            # Flush events
            if iteration - start_iter > 5 and ((iteration + 1) % cfg.SOLVER.WRITE_PERIOD == 0 or iteration == max_iter - 1): # originally writing every 20 iterations
...
def main(args):
    ...
    if args.eval_only:
        ...
        dataset_paths = [os.path.join(os.environ['DATA_DIR'], 'datasets', 'Omni3D', name + '.json') for name in cfg.DATASETS.TRAIN]
