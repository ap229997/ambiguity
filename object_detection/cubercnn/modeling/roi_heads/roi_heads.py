...
class ROIHeads3D(StandardROIHeads):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # load positional encoding config and remove from kwargs
        # since it is not a standard detectron2 config
        self.pos_enc = kwargs.pop('pos_enc')
        self.n_freq_pos_enc = kwargs.pop('n_freq_pos_enc')
        ...
        if self.pos_enc is not None:
            if 'center+corner_latent' in self.pos_enc:
                self.pos_enc_dim = 4 * 5 * self.n_freq_pos_enc
            elif 'dense_latent' in self.pos_enc:
                self.pos_enc_dim = 4 * self.n_freq_pos_enc

            self.feat_resolution = cube_pooler.output_size
            feat_dim = self.box_head.fc1.in_features // (self.feat_resolution[0] * self.feat_resolution[1])
            feat_conv_dim = feat_dim + self.pos_enc_dim
            self.pos_enc_feat_conv = nn.Sequential(
                nn.Conv2d(feat_conv_dim, feat_dim, kernel_size=1, stride=1, padding=0),
    ...
    def _init_cube_head(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        return {
            ...
            'pos_enc': cfg.MODEL.ROI_CUBE_HEAD.POS_ENC,
            'n_freq_pos_enc': cfg.MODEL.ROI_CUBE_HEAD.N_FREQ_POS_ENC,
        }
    ...
    def forward(self, images, features, proposals, Ks, im_scales_ratio, targets=None):
        ...
        max_h = max([dim[0] for dim in im_dims])
        max_w = max([dim[1] for dim in im_dims])
        ...
        if self.training:
            ...
            if self.loss_w_3d > 0:
                instances_3d, losses_cube = self._forward_cube(features, proposals, Ks, im_dims, im_scales_ratio, size=(max_h, max_w))
            ...
            if self.loss_w_3d > 0:
                pred_instances = self._forward_cube(features, pred_instances, Ks, im_dims, im_scales_ratio, size=(max_h, max_w))
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances]):
        ...
        proposal_boxes, tensor_boxes = [], []
        for x in proposals:
            proposal_boxes.append(x.proposal_boxes)
            tensor_boxes.append(x.proposal_boxes.tensor)
    ...
    def compute_pos_enc(self, boxes, intrx, all_im_dims):
        num_boxes = boxes.shape[0]

        if 'center+corner_latent' in self.pos_enc:
            center = (boxes[:, :2] + boxes[:, 2:]) / 2.0
            # print (center.shape, intrx.shape, boxes.shape)
            angle_x, angle_y = torch.atan2(center[:, 0] - intrx[:, 0, 2], intrx[:, 0, 0]), torch.atan2(center[:, 1] - intrx[:, 1, 2], intrx[:, 1, 1])
            center_angle = torch.stack([angle_x, angle_y], dim=-1)
            center_pos_enc = self.compute_center_pos_enc(center_angle)

            corner = torch.stack([boxes[:, 0], boxes[:, 1], boxes[:, 0], boxes[:, 3], boxes[:, 2], boxes[:, 1], boxes[:, 2], boxes[:, 3]], dim=-1)
            corner = corner.view(-1, 4, 2)
            corner = torch.stack([corner[... ,0] - intrx[:, 0, 2].unsqueeze(1), corner[..., 1] - intrx[:, 1, 2].unsqueeze(1)], dim=-1)
            intrx_denom = torch.stack([intrx[:, 0, 0], intrx[:, 1, 1]], dim=-1).unsqueeze(1)
            corner_angle = torch.atan2(corner, intrx_denom).view(num_boxes, -1)
            corner_pos_enc = self.compute_corner_pos_enc(corner_angle)

            ang_enc = torch.cat([center_pos_enc, corner_pos_enc], dim=-1)

        elif 'dense_latent' in self.pos_enc:
            downsample_factor = 1
            if 'ds' in self.pos_enc: downsample_factor = self.downsample_factor
            
            new_boxes = (boxes / downsample_factor).long()
            max_w, max_h = new_boxes[:, 2].max(), new_boxes[:, 3].max()
            max_dim = max(max_w, max_h)
            intrx = intrx / downsample_factor
            
            # create a dense grid of pixels
            x_grid, y_grid = torch.arange(0, max_dim+1).unsqueeze(0).repeat(num_boxes, 1), torch.arange(0, max_dim+1).unsqueeze(0).repeat(num_boxes, 1)
            x_grid, y_grid = x_grid.to(boxes.device), y_grid.to(boxes.device)
            x_pix = (x_grid - intrx[:, 0, 2].unsqueeze(1)) / intrx[:, 0, 0].unsqueeze(1)
            y_pix = (y_grid - intrx[:, 1, 2].unsqueeze(1)) / intrx[:, 1, 1].unsqueeze(1)
            x_grid = x_pix.unsqueeze(2).repeat(1,1,max_dim+1)
            y_grid = y_pix.unsqueeze(1).repeat(1,max_dim+1,1)
            angle = torch.stack([x_grid, y_grid], dim=1)

            rel_angle = torch.zeros_like(angle)
            rel_mask = torch.zeros_like(x_grid)
            for j in range(num_boxes):
                rel_angle[j, :angle[j].shape[0], :angle[j].shape[1], :angle[j].shape[2]] = angle[j]
                rel_mask[j, :angle[j].shape[1], :angle[j].shape[2]] = 1

            ang_enc = self.compute_dense_pos_enc(rel_angle, mask=rel_mask)
    ...
    def _forward_cube(self, features, instances, Ks, im_current_dims, im_scales_ratio, size: Tuple[int, int] = None):
        ...
        if self.pos_enc is not None:
            tensor_boxes = []
            bz = len(proposal_boxes_scaled)
            for i in range(bz):
                tensor_boxes.append(proposal_boxes_scaled[i].tensor)

            num_boxes = len(tensor_boxes[0])
            all_boxes = torch.cat(tensor_boxes, dim=0)

            intrx = Ks_scaled_per_box
            
            all_im_dims = []
            for i in range(bz):
                curr_boxes = len(tensor_boxes[i])
                all_im_dims.append(torch.tensor(im_current_dims[i]).repeat((curr_boxes,1)))
            all_im_dims = torch.cat(all_im_dims, dim=0).to(all_boxes.device)

            pos_enc = self.compute_pos_enc(all_boxes, intrx, all_im_dims)

            h, w = self.feat_resolution
            if 'center+corner_latent' in self.pos_enc:
                pos_enc = pos_enc.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, h, w)
            elif 'dense_latent' in self.pos_enc:
                pos_enc = F.interpolate(pos_enc, size=(h, w), mode='bilinear', align_corners=True)
            
            cube_features = cube_features.view(n, -1, h, w)
            cube_features = torch.cat([cube_features, pos_enc], dim=1)
            cube_features = self.pos_enc_feat_conv(cube_features)
            cube_features = cube_features.flatten(1)
