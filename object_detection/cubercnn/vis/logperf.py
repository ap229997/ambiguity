...
def print_ap_analysis_histogram(results):
    ...
    metric_names = ["AP2D", "AP3D", "AP3D@25", "AP3D@50", "AP3D@70", "AP3D-N", "AP3D-M", "AP3D-F"]
    N_COLS = 10
    data = []
    for name, metrics in results.items():
        ...
        data_item = [name, metrics["iters"], metrics["AP2D"], metrics["AP3D"], metrics["AP3D@25"], metrics["AP3D@50"], metrics["AP3D@70"], metrics["AP3D-N"], metrics["AP3D-M"], metrics["AP3D-F"]]
        data.append(data_item)
    table = tabulate(
        data,
        # headers=["Dataset", "#iters", "AP2D", "AP3D", "AP3D@15", "AP3D@25", "AP3D@50", "AP3D-N", "AP3D-M", "AP3D-F"],
        headers=["Dataset", "#iters", "AP2D", "AP3D", "AP3D@25", "AP3D@50", "AP3D@70", "AP3D-N", "AP3D-M", "AP3D-F"],
        tablefmt="grid",
        numalign="left",
        stralign="center",
    )
