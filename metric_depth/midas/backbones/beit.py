...
def forward_beit(pretrained, x, **kwargs):
    return forward_adapted_unflatten(pretrained, x, "forward_features", **kwargs)
...
def beit_forward_features(self, x, **kwargs):
    """
    ...
    if kwargs.get('tf_pos_enc', None) is not None:
        tf_pos_enc = kwargs.get('tf_pos_enc')
        x[:, :-1] = x[:, :-1] + tf_pos_enc
